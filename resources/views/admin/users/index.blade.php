    ...
    <div class="p-6" x-data="{
        ...
        get filteredUser() {
            return this.user.filter(item => {
                const matchesSearch = item.email.toLowerCase().includes(this.search.toLowerCase()) ||
                    item.nama.toLowerCase().includes(this.search.toLowerCase()) ||
                    (item.nim && item.nim.toLowerCase().includes(this.search.toLowerCase())) ||
                    (item.nidn && item.nidn.toLowerCase().includes(this.search.toLowerCase()));
                        ...
                        <form method="GET" class="space-y-4">
                            ...
                            <div>
                                ...
                                <select id="role" name="role"
                                    ...
                                    <option value="3" {{ request('role') == '3' ? 'selected' : '' }}>Mahasiswa
                                    </option>
                                </select>
                            </div>
        ...
        <x-table>
            <x-slot name="head">
                <tr>
                    ...
                    <th class="px-4 py-3 text-center">NIM/NIDN</th>
                ...
                <template x-for="item in filteredUser" :key="item.id_user">
                    <tr class="even:bg-gray-50 hover:bg-gray-100">
                        ...
                        <td class="px-4 py-3 text-gray-600 text-center">
                            <span x-text="item.nim || item.nidn || '-'"></span>
                        </td>
                        <td class="px-4 py-3 text-gray-600 text-center">
                            <template x-if="item.role == 1">
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                    Admin
                                </span>
                            </template>
                            <template x-if="item.role == 2">
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                    Dosen
                                </span>
                            </template>
                            <template x-if="item.role == 3">
                                <span
        ...
        <x-modal show="showDetailModal" title="Detail Akun">
            ...
            <div class="flex justify-center mb-8">
                <div class="relative">
                    <!-- Foto -->
                    <img src="{{ asset('img/default-avatar.png') }}" alt="Foto Profil"
            ...
            <div class="grid mt-4 grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                <div class="bg-gray-50 p-4 rounded-lg shadow-sm">
                    ...
                    <p class="font-medium text-gray-800">
                        <template x-if="selectedUser.role == 1">
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                Admin
                            </span>
                        </template>
                        <template x-if="selectedUser.role == 2">
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                Dosen
                            </span>
                        </template>
                        <template x-if="selectedUser.role == 3">
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                Mahasiswa
                            </span>
                ...
                <div class="bg-gray-50 p-4 rounded-lg shadow-sm">
                    <p class="text-xs text-gray-500">NIM/NIDN</p>
                    <p class="font-medium text-gray-800">
                        <span x-text="selectedUser.nim || selectedUser.nidn || '-'"></span>
                    </p>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg shadow-sm">
                    <p class="text-xs text-gray-500">Tanggal Dibuat</p>
                    <p class="font-medium text-gray-800"
                        x-text="new Date(selectedUser.created_at).toLocaleDateString('id-ID')"></p>
                </div>
...
<x-modal show="showEditModal" title="Edit Akun">
    <form :action="`{{ url('/admin/user/edit') }}/${selectedUser.id_user}`" method="POST">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="">
            ...
            <div class="">
                ...
                <div class="relative">
                    <input x-model="selectedUser.email" type="email" placeholder="Email" name="email"
                        class="w-full px-3 py-2 text-sm border rounded-md focus:ring focus:border-blue-500"
                        required>
                    <button type="button" @click="selectedUser.email=''" x-show="selectedUser.email && selectedUser.email.length > 0"
                        class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            {{-- password --}}
            <div class="col-span-2">
                <label for="Password" class="block text-sm font-medium">Password (Biarkan kosong jika tidak
                    ingin diubah)</label>
                <div class="relative">
                    <input :type="showPassword ? 'text' : 'password'" x-model="selectedUser.password"
        ...
        <div x-show="showDeleteModal" @click.away="showDeleteModal = false" x-transition
            ...
            <div class="bg-white rounded-xl shadow-2xl w-96 p-6 text-center">
                ...
                <form :action="`{{ url('/admin/user') }}/${selectedUser.id_user}`" method="POST" x-ref="deleteForm">
                    @csrf
                    @method('DELETE')
