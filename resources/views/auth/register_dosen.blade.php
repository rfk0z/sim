    ...
    <div x-data="{
        ...
        <div class="flex w-full md:w-1/2 p-8 relative z-10 flex-col justify-center">
            ...
            <form @submit.prevent="if (!agree) { alert('Setujui Terms & Conditions!'); return; } $el.submit();" method="POST"
                action="{{ route('register.dosen') }}" class="bg-white p-8 rounded shadow-md w-full max-w-md space-y-4">
                @csrf
                <h2 class="text-3xl font-bold text-blue-700 text-center">Registrasi Dosen</h2>

                <input type="text" name="nidn" x-model="nidn" placeholder="NIDN" required
                ...
                <input type="text" name="jabatan" x-model="jabatan" placeholder="Jabatan" required
                    value="{{ old('jabatan') }}"
                    class="w-full p-3 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500" />
            ...
            <div class="text-center space-y-6">
                ...
                <div class="space-y-3">
                    ...
                    <a href="{{ route('register.mahasiswa') }}"
                        class="block border-2 border-white text-white px-6 py-3 rounded-full font-semibold hover:bg-white hover:text-blue-700 transition">Daftar sebagai Mahasiswa</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="flex md:hidden w-full min-h-screen items-center justify-center p-4">
        <div x-data="{
            ...
            <div class="text-center">
                ...
                <h2 class="text-xl font-bold text-blue-700">Registrasi Dosen</h2>
            </div>
            ...
            <form @submit.prevent="if (!agree) { alert('Setujui Terms & Conditions!'); return; } $el.submit();" method="POST" action="{{ route('register.dosen') }}" class="space-y-3">
                @csrf

                <input type="text" name="nidn" x-model="nidn" placeholder="NIDN" value="{{ old('nidn') }}" maxlength="10" required
                    class="w-full p-3 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500">

                <input type="text" name="nama" x-model="nama" placeholder="Nama Lengkap" value="{{ old('nama') }}" required
                    class="w-full p-3 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500">

                <input type="text" name="jabatan" x-model="jabatan" placeholder="Jabatan" value="{{ old('jabatan') }}" required
                    class="w-full p-3 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <button type="submit"
                    class="w-full bg-blue-700 text-white py-3 rounded hover:bg-blue-800 transition font-bold">Daftar Dosen</button>
