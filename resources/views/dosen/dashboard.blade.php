<x-dosen-layout>
    ...
    <x-dosen-header :bimbinganBaru="$bimbinganBaru" />
    <div class="pt-30">
    {{-- Stat Cards --}}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 top-4 right-4 z-50">
        <x-stat-card label="Total Bimbingan Saya" :value="$statistik_bimbingan['total_dosen']" color="text-blue-600">
        ...
        <x-stat-card label="Bimbingan Bulan Ini" :value="$statistik_bimbingan['bulan_ini']" color="text-green-600">
            <svg xmlns='http://www.w3.org/2000/svg' width='28' height='28' viewBox='0 0 24 24' fill='none'
                stroke='currentColor' stroke-width='1.714' stroke-linecap='round' stroke-linejoin='round'
        ...
        <x-stat-card label="Bimbingan Selesai" :value="$statistik_bimbingan['selesai']" color="text-emerald-600">
            <svg xmlns='http://www.w3.org/2000/svg' width='28' height='28' viewBox='0 0 24 24' fill='none'
                stroke='currentColor' stroke-width='1.714' stroke-linecap='round' stroke-linejoin='round'
        ...
        <x-stat-card label="Bimbingan Baru" :value="$bimbinganBaru->count()" color="text-orange-600">
            <svg xmlns='http://www.w3.org/2000/svg' width='28' height='28' viewBox='0 0 24 24' fill='none'
                stroke='currentColor' stroke-width='1.714' stroke-linecap='round' stroke-linejoin='round'
        ...
        <div class="bg-white p-6 rounded-2xl shadow" x-data="{
            ...
            renderChart() {
                ...
                this.chart = new Chart(ctx, {
                    type: 'pie',
                    data: {
                        labels: ['Pending', 'Valid (Selesai)', 'Invalid'],
                        datasets: [{
                            data: [this.data.pending, this.data.valid, this.data.invalid],
            ...
            <div class="text-sm text-gray-600 space-y-2 mt-4">
                <div class="flex items-center justify-between">
                    ...
                    <span class="font-semibold text-gray-800">{{ $pieChartData['pending'] }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <span class="inline-block w-3 h-3 rounded-sm bg-green-500"></span>
                        <span>Valid (Selesai)</span>
                    </div>
                    <span class="font-semibold text-gray-800">{{ $pieChartData['valid'] }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                    ...
                    <span class="font-semibold text-gray-800">{{ $pieChartData['invalid'] }}</span>
                </div>
            </div>
        </div>
                ...
                <div class="space-y-4">
                    @foreach($recentBimbingan as $bimbingan)
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                            <div>
                                <h3 class="font-medium text-gray-800">{{ $bimbingan->mahasiswa->nama }}</h3>
                                <p class="text-sm text-gray-600">NIM: {{ $bimbingan->mahasiswa->id_nim }}</p>
    ...
    <script>
        ...
        new Chart(monthlyCtx, {
            type: 'line',
            data: {
                labels: monthlyData.map(item => item.bulan_short),
                datasets: [
                    {
