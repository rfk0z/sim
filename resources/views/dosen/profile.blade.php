            ...
            <div class="grid md:grid-cols-3 gap-10">
                ...
                <div class="md:col-span-2 space-y-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" id="nama" name="nama"
                            value="{{ old('nama', $user->dosen ? $user->dosen->nama : $user->username) }}"
                            class="w-full px-4 py-2 border rounded-xl focus:ring-2 focus:ring-blue-500" required />
                        @error('nama')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" value="{{ $user->dosen->id_nidn ?? '' }}" readonly
                            class="w-full px-4 py-2 border rounded-xl bg-gray-100 cursor-not-allowed" />
                    </div>
                    <div class="space-y-3">
                        <label for="fakultas" class="block text-sm font-semibold text-gray-700">Fakultas</label>
                        <select id="fakultas" name="fakultas"
                            class="w-full px-4 py-2 border rounded-xl focus:ring-2 focus:ring-blue-500" required>
                            <option value="">Pilih Fakultas</option>
                            @foreach ($fakultasList as $fakultas)
                                <option value="{{ $fakultas }}"
                                    {{ $user->dosen && $user->dosen->fakultas == $fakultas ? 'selected' : '' }}>
                                    {{ $fakultas }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-6 flex flex-col items-center">
                    <div class="relative w-36 h-36">
                        <img src="{{ $user->foto ? asset($user->foto) : '/images/profil-default.jpg' }}"
